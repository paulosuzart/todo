...
class TD {
    ...
    def add(form : NodeSeq) = {
        ...
        bind("todo", form, "priority" -> todo.priority.toForm,
    ...
    private def desc(td: ToDo, reDraw: () => JsCmd) = swappable(<span>{td.desc}</span>,
                                                                <span>{ajaxText(td.desc, v => {td.desc(v).save; reDraw()})}
                                                                </span>)
    ...
    toShow.flatMap(td =>
        bind("todo", html,
             ...
             "priority" -> ajaxSelect(ToDo.priorityList,
                                      Full(td.priority.toString), v => {td.priority(v.toInt).save; reDraw()}),
             "desc" -> desc(td, reDraw)))
